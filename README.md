...
jobs 명령어는 현재 쉘 세션에서 실행 중인 작업들의 상태를 확인한다.
...
<img width="532" height="98" alt="img1 daumcdn" src="https://github.com/user-attachments/assets/2df7fedd-7fc9-4f79-a718-42e993868de7" />
...
|항목(용)|설명|
|----|----|
|[1], [2]|작업 번호: 쉘이 관리하는 번호|
|+, -|현재 작업 표시 : (+: 현재 작업 fg나 bg 명령을 인자 없이 썼을 때 대상이 됨)  (- : 이전 작업 + 작업이 끝나면 이 작업이 +가 됨)|
|Running|현재 실행 중인 상태|
|Stopped|일시 중지된 상태|
|Done|작업이 정상적으로 완료됨|
|Command|실행된 명령어 내용|
...
**주요 옵션들**
* jobs -l: 프로세스 ID (PID)를 함께 표시

* jobs -p: 작업 번호는 제외하고 오직 PID만 출력

* jobs -r: 현재 실행 중(Running)인 작업만 출력

* jobs -s: 현재 중지된(Stopped) 작업만 출력
...
-> 종료해달라고 요청도 가능하고 강제로 종료하라고 명령할 수 도 있다.


**kill 명령어의 실행법**


`$ kill [옵션/시그널] [PID 또는 작업번호] `

`$ kill [PID 또는 작업번호]`
를하면 기본적으로 15번 SIGTERM을 보낸다.
...
|번호|이름|의미|설명|
|---|---|---|---|
|15|SIGTERM|Terminate|프로세스에게 종료를 요청 프로세스는 하던 일을 정리하고 스스로 종료|
|9|SIGKILL|kill|강제 종료. 프로세스의 의사를 묻지 않고 즉시 메모리에서 제거|
|2|SIGINT|Interrupt|실행 중인 작업을 중단|
|1|SIGHUP|Hangup|재시작하거나 설정을 다시 불러오라고 시킨다.|
